# Auth Events Monitor

## Como rodar o projeto


## Decisões técnicas

* -Decidi utilizar o SSE pela liberdade, agilidade e compatibilidade com a estrutura do projeto.
* -Apliquei o Express para Facilitar a criação do servidor com Node e maximizar a comunicação com diferentes URLs.
* -Apliquei o Cors para Gerenciar a comunicação entre as portas, React e Node.
* -Criei o projeto pelo NPX como descrito na documentação do React.   
* -Poderia ter Utilizado o (vite) para criar um projeto mais limpo e escalavel, mas decidi seguir com o npx por está mais acostumado com a criação padrão dos projetos em React.
* -Primeiro estruturei todas as pastas e files necessarias para iniciar o projeto.
* -Reutilizei um box de Login ja criado por mim em outro projeto para adiantar o processo de desenvolvimento.
* -Desenvolvi a base dos componentes da tela para otimizar as etapas seguintes do projeto.
* -Inicializei a configuração e primeiros testes do servidor backend.
* -Conexão finalizada e inicio de criação das rotas.
* -mudei totalmente as estrutura do React, percebi que estava usando um hmlt e o type quase vanila, lendo as documentações percebi a forma certa de utilizar o react e maximizar a produção.
* -Rota de Login 100% funcional e retornando o status code e o evento atribuido a cada status.
* -Configurei a lista de eventos do frontend para receber os eventos em tempo real.

## Diferença entre autenticação e autorização


## Fluxo de eventos



## Pontos de melhoria
* -implementar o token JWT
* -implementar algum banco de dados para armazenar os Usuarios.
* -criar um formulario de cadastro para cadastrar novos usuarios.

## Dificuldades encontradas 
* -problemas com a estrutura do React por fazer muito tempo sem usar, tive problemas no inicio.
* -definir o formato da entrada de dados dos eventos.

## Bibliotecas utilizadas
* -React
* -node.js
* -express
* -cors